...
export default class Posts {
    ...
    async setLike(): Promise<boolean | undefined> {
        if (!this.post || !this.session) return;
        const query = gql`mutation Liked($where: PostWhereUniqueInput!, $data: PostUpdateInput!) {
        ...
        return await liked.data.updatePost?.isLiked;
    };
